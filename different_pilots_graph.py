...
def various_len_pilots():
    Results = {}
    len_set = len(set_num_frame)
    Results['mse_our'] = np.zeros(len_set, )
    N_devices = 10 
    for num_frame in set_num_frame:
        params_system = (num_antenna_bs, N_IRS, N_devices)
        idx = set_num_frame.index(num_frame)
        Pilots_Length = num_frame*N_devices
        print('Length of Pilot :%d' % Pilots_Length)
        print('---------')
...
Max_iter = 20
Training_size, Test_size = Max_iter*1000, 1024
set_num_frame = [1, 4, 8, 16, 32]

t1 = time.time()
dir = './data/%s/Pilots_%s_noise-%d'%(date.today().strftime('%Y-%m-%d'), str(time.strftime('%H_%M_%S')),noise_power_db)
# dir = r'.\data\2021-12-08\Pilots_00_12_32_noise--75'
if not os.path.exists(dir):
    os.makedirs(dir)
Result = various_len_pilots()
